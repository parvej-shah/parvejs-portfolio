      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-x-6 gap-y-8 px-5 min-[420px]:grid-cols-2 lg:grid-cols-4">
        {content.items.map((s, i) => (
          <Reveal
            key={s.label}
            delay={i * 80}
            className="group rounded-2xl border border-line bg-ink-2/70 dark:bg-transparent p-5 sm:p-6 shadow-xs transition-all duration-300 hover:border-brand/40 hover:shadow-md"
          >
            ...
            <p className="mt-3 max-w-[16rem] text-sm text-muted-foreground leading-relaxed">{s.label}</p>
          </Reveal>
        ))}
      </div>
    </section>
